...
Every `CHANGE` Task identifies its modified repositories. Each modified repository resolves one immutable Base Branch: the repository-specific Task Contract Base Branch when provided, otherwise that repository's Default Base Branch. For that repository, the same Base Branch is the Task Branch creation source and final merge target. Affected but unmodified repositories may contribute inspection or verification evidence but do not resolve lifecycle artifacts solely because they participate in the Task.
...
- `CHANGE`: repository modification is authorized only within scope, exactly one dedicated `task/*` Task Branch is required in each modified repository, and one Task-level Independent Review of the complete Task Review Target is mandatory before completion.
...
The Codex Task Prompt contains an Authoritative Task Core: Goal, Scope, Out of Scope, Requirements, material Constraints / Decisions, and Acceptance Criteria, with related task-specific context as the template defines. It carries transient engineering intent that cannot reliably be recovered from repository state. Transfer into the Codex Review Prompt is copying, not regeneration. Codex A MUST copy the entire Core exactly and MUST NOT rewrite, summarize, normalize, reformat, correct, reorder, abbreviate, expand, translate, or otherwise modify its wording, punctuation, Markdown, whitespace, ordering, or content. Exact textual equality, not semantic equivalence, is required even when the original contains an error. If the handoff medium cannot preserve the Core exactly, use a representation or transfer mechanism that can. Codex A records any implementation interpretation, clarification, deviation, implementation evidence, verification evidence, navigation, and previous-review state separately as Codex A-produced context.

The Task and Review Prompt templates define canonical Original and Embedded Core boundaries. Equality operands are all raw text inside those boundaries; boundary markers are framing. Extraction and comparison perform no trim, dedent, newline normalization, Markdown normalization, or other preprocessing. The Review Prompt MUST actually embed the complete Core. Empty content, a placeholder, TODO, copy instruction, Original Task Prompt reference, repository path, attachment, link, summary, excerpt, or other external dependency is not an embedded Core. The Review Prompt must be self-sufficient for authoritative task intent; Codex B must not need another artifact to obtain or reconstruct it.

The Review Prompt visibly separates the inherited Authoritative Task Core from Codex A-produced engineering information. It supplies a fresh Codex B session with the unchanged Task Core, exact Git review targets, the material implementation delta and decisions, Codex A verification evidence, useful navigation, deviations, and previous-review state when applicable. The Task artifact need not be persisted, and Codex B must not reconstruct transient task intent from Git history. Before presenting a generated Review Prompt to the human, Codex A MUST perform the runtime pre-handoff gate defined by the Review Prompt template: validate both boundaries, complete actual non-substitute embedding, and exact raw textual equality with the Original Core. A failure MUST be corrected or regenerated; Codex A MUST NOT present a failing prompt as the formal handoff. This validation creates no additional formal artifact.
...
Same-task Codex A1 → Codex A2 continuation is allowed without another formal artifact. It preserves Task ID, Task Type, Task Contract identity, Task Branches, and Review Attempt count. Repository state, commits, Task Contract, and Review Reports are the system of record. Checkpoint commits are allowed but are not Review Commits.
...
Codex A must implement, verify, commit, and provide a Codex Review Prompt identifying the Base Branch, Base Commit, Task Branch, and Review Commit for every modified repository. Together, those repository-specific targets form the complete Task Review Target. Each Review Commit must be its repository's Task Branch `HEAD`; working-tree-only review is invalid. Affected but unmodified repositories require no Review Commit. Once the prompt is ready, Codex A MUST stop review execution in the Implementation Owner session. The next workflow action belongs to the human user, who explicitly creates the new Codex B session and transfers the prompt.
...
Codex B performs one Task-level review of the complete Task Review Target: every modified repository's `Base Commit..Review Commit` change set and repository state at its Review Commit, plus relevant cross-repository consistency. Codex A's implementation narrative is not evidence of correctness.
...
Approval is bound to the exact Approved Commit in every modified repository and to the complete Task Review Target. Any later repository modification or rebase invalidates approval and requires verification and a new Independent Review.
