...
DESIGN_CONVERSATION_TOKENS = (
    ...
    "COMPILE",
    "先扫描，再提问；先解决，再提问。",
    "先确定当前下一动作，再计算该动作的最小阻塞集",
    "`未确认` 是所有经用户授权且当前仍未解决的内容，不等同于当前最小阻塞集",
    "若结果为空，则不提问并执行下一动作",
    "不属于当前最小阻塞集的问题可以暂缓",
    "普通问题、普通对话和普通用户陈述不得直接新增、修改或删除",
    "没有用户回复不构成选项",
    "Decision ID",
    "`接受 / 待定 / 拒绝`",
    "紧接的那一条用户回复中有效",
    ...
    "决策单元立即过期",
    "字段 + 创建该条目的 Decision ID",
    "同一 Decision ID 可以在 `已确认` 和 `未确认` 各出现一次",
    "操作｜字段｜决策 ID：变更前 → 变更后",
    "`新增`、`修改`、`删除`、`不变`",
    "逐字应用预先展示的状态文字",
    "删除使用原 Decision ID 的旧 `未确认` 条目",
    "当前决策单元的 Decision ID 新增 `已确认` 条目",
    "Checkpoint",
    "目标\n已确认\n未确认\n状态变更记录",
    "不得包含 `下一步` 字段",
    "不得使用 `保持上一 Checkpoint`、`同上`、`其余不变`",
    "不得记录 `不变`、`目标` 的变化",
    "也不得跨多个 Checkpoint 累积",
    "不是对话恢复机制",
    "每 10 个用户回合必须创建",
